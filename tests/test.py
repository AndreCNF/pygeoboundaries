from pygeoboundaries import geoboundaries as gbd
...
def test_get_adm():
    assert type(gbd.get_adm("sn", "ADM0")) == FeatureCollection
    assert type(gbd.get_adm("Senegal", "ADM0")) == FeatureCollection
    for i in range(6):
        assert type(gbd.get_adm("France", i)) == FeatureCollection
    assert type(gbd.get_adm("Vanuatu", -1)) == FeatureCollection
