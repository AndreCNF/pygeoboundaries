...
from pygeoboundaries import countries_iso_dict
from pygeoboundaries import iso_codes
...
def _is_valid_adm(iso3, adm: str) -> bool:
    html = _session.get(
        "https://www.geoboundaries.org/api/current/gbOpen/{}/".format(iso3), verify=True
    ).text
...
def get_metadata(territory: str, adm: str | int) -> dict:
    ...
    """
    return _session.get(
        _generate_url(territory, adm), verify=True
...
def get_adm(territories: str | List[str], adm: str | int, simplified=True) -> dict:
    ...
    geojsons = [
        geojson.loads(_get_data(i, adm, simplified))["features"][0] for i in territories
    ]
